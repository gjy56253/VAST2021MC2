    ...
    return html.Div([
        dbc.Row([
            ...
            dbc.Col([
                ...
                html.P(
                    "2014年1月13日上午，信用卡6816连续出现两笔关键交易:07:43，该卡首先在 Brew've Been Served 完成消费,随后08:01又在 Kronos Mart 产生第二笔交易。,然而车辆20的GPS与停车记录显示，其仅在07:51前后发生过一次短暂停留，之后便继续沿既定路线行驶,车辆活动轨迹未覆盖两处，推测其信用卡存在被他人使用的情况。",
                    className="text-muted"
                ),
                ...
                html.P(
                    "信用卡9551于2014年1月13日全天产生两笔消费记录，上午时段的交易虽然与车辆1的轨迹重合故暂时认定为正常出行活动。然而，晚间时段出现了明显的时空矛盾：19:20该信用卡在 Drydo's Autosupply n'More 产生消费记录，而车辆1在该时刻附近并无对应轨迹；仅10分钟后的19:30，信用卡又出现在 Ouzeri Elian 完成另一笔交易，此时车辆轨迹却恰好出现在后者附近，说明交易记录与车辆活动记录之间存在明显断裂，进一步验证了任务一识别出的异常时空行为并非数据误差，而是信用卡脱离持有人控制、被多人共享使用的高风险信号。",
                    className="text-muted"
                ),
                ...
                html.P(
                    "信用卡7108于当天发生三笔消费记录，结合车辆12的停车轨迹分析发现，上午07:30与07:44两笔连续消费均处于车辆停车记录之间；晚间消费虽未与车辆停车记录完全重合，但两者之间仍存在合理时间窗口。目前证据仅能说明车辆轨迹与消费行为存在一定程度的不完全匹配，尚不足以单独认定为信用卡代刷或多人共用行为。",
                    className="text-muted"
                ),
                ...
                dbc.Card(dbc.CardBody([
                    ...
                    dbc.Alert([
                        ...
                        html.Ul([
                            ...
                            html.Li("6816：车辆离开后仍产生消费记录，疑似信用卡借用或代刷。")
