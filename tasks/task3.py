        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                dbc.Card(dbc.CardBody([
                    ...
                    html.P("浅橙色区块表示物理轨迹异常，即存在车辆轨迹与超过 1 张信用卡消费记录高度重合的情况，推测该车主拥有多张信用卡或存在私下借用专属车辆与代刷行为；浅蓝色区块表示财务逻辑异常，反映了任务一识别出的信用卡与会员卡的交叉互用及信息隐匿现象；浅红色区块表示双重高危异常，揭示车辆与信用卡同时发生跨界共享的复杂关联；灰色区块代表幽灵与公共资源集群，归纳了未分配车辆员工由于使用公共卡车或无可见轨迹而无法进行独立空间定位的样本集合。", className="text-muted small")
                ]), className="mb-5 bg-light"),

                # 3.2.2 概率流向异质关联拓扑
                html.H4("3.2.2 概率流向异质关联拓扑", id="section-3-2-2", className="mt-4 mb-3"),
                html.P(
                    "本拓扑图根据轨迹与消费重合率全面连接了物理轨迹与财务流水中存在多维从属可能性的矛盾节点，为直观呈现推断置信度的量化衰减，采用流向线条的透明度深浅映射该关联的数学概率，其中路径色阶越深表明该信用卡、员工姓名与福利卡之间的多重对齐概率越高，置信度越强；相反，半透明的微弱流向则提示该链路属于偶发性异常或低置信度的跨界噪音，为后续的深层审计提供了全局流向依据。",
                    className="text-justify text-muted"
                ),
                dbc.Card(dbc.CardBody([
                ...
                html.P(
                    "我们在车辆分配表中发现了 9 名未分配公司车辆的基层或现场员工，而系统匹配后恰好残留了 7 张与公共卡车轨迹高度重合的信用卡以及 2 张彻底不产生任何轨迹的信用卡。",
                    className="text-justify text-muted"
                ),
                dbc.Card(dbc.CardBody([
                    html.H6("车辆未分配引起的底层系统数据盲区", className="fw-bold text-secondary mb-3"),
                    html.P(
                        "这 9 张无法定位具体车主的信用卡数量与公司内没有配车的 9 名员工实现了数字对齐，但由于卡车属于多人多部门混合调度资源且无轨迹人员不可见，导致数据无法在此集群内部进行进一步的 1 对 1 解耦。",
                        className="text-muted"),

                    html.Div([
                        html.Strong("无车人员名单"), html.Br(),
                ...
                html.Ul([
                    html.Li([
                        ...
                        "系统中活跃的信用卡数量远超公司官方登记的员工总数，这种基数不对等暗示了潜在的一名员工拥有多张信用卡、新员工登记遗漏或外部人员用卡现象，导致部分资产在物理上无法找到合法的归属锚点。"
                    ], className="mb-2"),
                    html.Li([
                        html.Span("底层物理轨迹盲区：", className="fw-bold"),
                        "部分人员未被分配车辆或完全没有产生任何可用的 GPS 定位记录，这种未知数据的存在使得底层验证依据完全缺失，无法为财务消费提供时空碰撞比对。"
                    ])
                ], className="text-muted"),
